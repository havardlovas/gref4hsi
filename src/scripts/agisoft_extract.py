...
def extract_pose(chunkName, chunks, csv_name, crs_export, config):
        ...
        if chunk.label in chunkName:
            ...
            for cam in cameras:
                ...
                if cam.center != None:
                    ...
                    # This should be OK
                    r_scipy = Rotation.from_matrix(np.asarray(R_cam_ecef).reshape((3, 3)))
                    rotz, roty, rotx = r_scipy.as_euler("ZYX", degrees=True)


                    # Then we define a local frame

                    #R = m * T * cam.transform * MS.Matrix().Diag([1, -1, -1, 1])
                    #row = list()
                    ...
                    ##print(R)
                    #yaw, pitch, roll = MS.utils.mat2ypr(R)  # estimated orientation angles in yaw pitch roll
...
def extract_model(chunkName, chunks, model_name):
    for chunk in chunks:
        if chunk.label in chunkName:
            cameras = chunk.cameras
            if not len(cameras):
...
def main(iniPath):
    ...
    if local:
        config = extract_pose(chunkName=chunkName, chunks=chunks, csv_name=csv_name, crs_export=chunk.crs, config = config)
    else:
        config = extract_pose(chunkName=chunkName, chunks=chunks, csv_name=csv_name, crs_export=crs_export, config = config)
