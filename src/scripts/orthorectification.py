...
def main(iniPath):
    ...
    h5_dir = config['Absolute Paths']['h5Dir']                                                                                            #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Input/H5/'

    # Paths for output data:
    # 1) The data cube locations
    envi_cube_dir = config['Absolute Paths']['orthoCubePaths']                                                                            #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Output/GIS/HSIDatacubes/'

    # 2) The RGB composite locations
    rgb_composite_dir = config['Absolute Paths']['rgbCompositePaths']                                                                     #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Output/GIS/RGBComposites/'

    # 3) The ancillary data locations
    anc_dir = config['Absolute Paths']['anc_paths']                                                                                       #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Output/GIS/AncillaryData/'

    # 3) The footprints
    footprint_dir = config['Absolute Paths']['footprintpaths']                                                                            #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Output/GIS/Footprints/'

    # The necessary data from the H5 file for resampling the datacube and composite (fwhm is optional)
    h5_path_point_cloud_ecef   = config['Georeferencing']['points_ecef_crs']                                                              #Oliver: 'processed/georef/points_ecef_crs'
    h5_path_radiance_cube      = config['HDF.hyperspectral']['datacube']                                                                  #Oliver: 'DATA/CUBE/cube'                            
    h5_path_wavelength_centers = config['HDF.calibration']['band2wavelength']                                                             #Oliver: 'DATA/CUBE/wavelength'
    try:
    ...
    anc_dict = config['Georeferencing']                                                                                                     #Oliver: 'processed/georef/ancillary'
    ...
    config_crs = SettingsCRS(epsg_geocsc=int(config['Coordinate Reference Systems']['geocsc_epsg_export']),                                 #Oliver: Create named tuple "config_crs"
    ...
    config_ortho = SettingsOrtho(ground_resolution = float(config['Orthorectification']['resolutionHyperspectralMosaic']),                  #Oliver: Create named tuple "config_ortho"               
    ...
    for filename in sorted(os.listdir(h5_dir)):
        if filename.endswith('h5') or filename.endswith('hdf'):
            ...
            h5_filename = h5_dir + filename                                                                                                 #Oliver: 'D:/Ocean Color Colab/Processing Step4/Flight_1/Input/H5/NyAalesundMinicruiserMEKF_1.h5'

            # Read the 3D point cloud, radiance cube 
            # Extract the point cloud
            point_cloud_ecef = Hyperspectral.get_dataset(h5_filename=h5_filename,                                                           #Oliver: Get point cloud from h5 file (saved in h5 cube during georeferencing) => 'processed/georef/points_ecef_crs' -> mesh intersections in ECEF - pos0
                                                         dataset_name=h5_path_point_cloud_ecef)
            # Need the radiance cube for resampling
            radiance_cube = Hyperspectral.get_dataset(h5_filename=h5_filename,                                                              #Oliver: Get radiance cube from h5 file => 'DATA/CUBE/cube' -> radiance cube not changed
                                                      dataset_name=h5_path_radiance_cube)

            wavelengths = Hyperspectral.get_dataset(h5_filename=h5_filename,                                                                #Oliver: Get wavelengths from h5 file => 'DATA/CUBE/wavelength' -> wavelengths (not changed)
            ...
            gisHSI = GeoSpatialAbstractionHSI(point_cloud=point_cloud_ecef,                                                                 #Oliver: create "GIS HSI" object
                                              transect_string=filename.split('.')[0],
                                              config_crs=config_crs)

            # The point cloud is transformed to the projected system
            gisHSI.transform_geocentric_to_projected(config_crs=config_crs)                                                                 #Oliver: Project point cloud from ECEF to UTM
